...
export default function KalyptoChat() {
    ...
    return (
        <>
            ...
            <m.button
                className="fixed bottom-6 left-6 z-[9999] w-14 h-14 rounded-full bg-gradient-to-r from-[#005A9C] to-[#00BFA5] text-white shadow-lg flex items-center justify-center hover:shadow-xl hover:scale-105 transition-all duration-300"
                onClick={() => setIsOpen(!isOpen)}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                aria-label="Open Cami AI Chat"
            >
                {isOpen ? (
            ...
            <AnimatePresence>
                {isOpen && (
                    <m.div
                        ...
                        className="fixed bottom-24 left-6 z-[9999] w-[90vw] md:w-[400px] h-[500px] max-h-[80vh] bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 flex flex-col overflow-hidden"
                    >
                        {/* Header */}
